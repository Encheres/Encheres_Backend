...
router.route("/filtered-auctions").get(async (req, res, next) => {
    const PAGE_SIZE = 5;
...
//for all the auctions if no timeline is passed (no find)

router.route("/all-auctions").get(async (req, res, next) => {
    const PAGE_SIZE = 5;
    const page = parseInt(req.query.page || "0");
    try {
        var auctions = await Auction.find({
...
router
    ...
    .get(async (req, res, next) => {
        try {
            var auction = await Auction.findById(req.params.auctionId).populate(
                "organizer",
                "name"
            );
            res.statusCode = 200;
    ...
    .put(auth, async (req, res, next) => {
        try {
            var auction = await Auction.findByIdAndUpdate(
    ...
    .delete(auth, async (req, res, next) => {
        try {
            var operation = await Auction.deleteOne({
...
router.route("/auctions/:auctionId/:itemId").get(async (req, res, next) => {
    try {
...
// Completed Auctions.
router.route("/completed-filtered-auctions").get(async (req, res, next) => {
    const PAGE_SIZE = 5;
    const page = parseInt(req.query.page || "0");
...
router.route("/completed-auctions").get(async (req, res, next) => {
    const PAGE_SIZE = 5;
    const page = parseInt(req.query.page || "0");
...
router.patch("/auctions/:auctionId/:itemId", async (req, res) => {
    try {
        ...
        if (
            !oldItem.items[index].bid.price ||
            parseFloat(oldItem.items[index].bid.price) <
            ...
            oldItem.chats.push({
                ...
                message: data.bid.price + " ETH: Competing Bid",
                time: date_time,
            });
            await oldItem.save();
    ...
    try {
        ...
        if (oldItem.items[index].sell.sold === false) {
            ...
            if (diff >= 120) {
                ...
                oldItem.items[index].sell.sold_bidder =
                    latest_bid.bidder.userId;
                ...
                oldItem.chats.push({
                    message_type: "sold",
                    message:
                        oldItem.items[index].name +
                        " sold to " +
                        oldItem.items[index].bid.bidder.anonymous_name,
                    time: date_time,
                });
                await oldItem.save();
